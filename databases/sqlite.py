...
class SQLite3Client:
    ...
    def create_table(self):
        ...
        self.cursor.execute("""CREATE TABLE IF NOT EXISTS working_chat(url TEXT, id_of_message INT, message TEXT, check_completely BOOL, PRIMARY KEY(url))""")
    ...
    def add_working_chat_into_table(self, url: str, id_of_message: int = 0, message: str = '', check_completely: bool = 0):
        try:
            self.cursor.execute("""INSERT INTO working_chat(url, id_of_message, message, check_completely) VALUES (?, ?, ?, ?)"""
    ...
    def update_id_of_message(self, url: str, id_of_message: int):
        self.cursor.execute(f"""UPDATE working_chat SET id_of_message = {id_of_message} WHERE url = "{url}" """)
        self.connection.commit()

    def update_message(self, url: str, message: str):
        self.cursor.execute(f"""UPDATE working_chat SET message = "{message}" WHERE url = "{url}" """)
        self.connection.commit()

    def update_check_completely(self, url: str):
        self.cursor.execute(f"""UPDATE working_chat SET check_completely = 1 WHERE url = "{url}" """)
        self.connection.commit()
    ...
    def get_id_of_message(self, url: str):
        self.cursor.execute(f"""SELECT id_of_message FROM working_chat WHERE url = "{url}" """)
