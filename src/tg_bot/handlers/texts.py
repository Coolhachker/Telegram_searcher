...
from src.tg_bot.Configs.templates import parser_is_working_message, parser_isnt_working_message, message_for_actions_over_ban_list


def handle_texts(dispatcher: Dispatcher, bot: Bot):
    @dispatcher.message(lambda message: NamesOfButtons.reply_button_chats == message.text)
    ...
    @dispatcher.message(lambda message: NamesOfButtons.comeback_button == message.text)
    ...
    @dispatcher.message(lambda message: NamesOfButtons.key_words_button == message.text)
    ...
    @dispatcher.message(lambda message: NamesOfButtons.parser_button == message.text)
    ...
    @dispatcher.message(lambda message: NamesOfButtons.ban_list_button == message.text)
    async def handle_ban_list_button(message: Message):
        await bot.send_message(message.chat.id, message_for_actions_over_ban_list, reply_markup=return_ban_list_keyboard())
