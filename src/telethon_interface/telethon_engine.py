...
class TelethonEngine:
    def __init__(self):
        ...
        url_of_chats = [url[2] for url in sqlite3_client.get_chats()]
        sqlite3_client.setup_working_chats(url_of_chats)
    ...
    async def center_of_processing_task(self, url: str):
        check_completely = sqlite3_client.get_check_completely(url)
        if check_completely == 0:
